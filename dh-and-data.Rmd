...
Next, we'll load our data into R in order to visualize it as a map. An important intermediate step to visualization is geocoding, or adding latitude and longitude coordinates to our `street address` column. In order to geocode, you will need to create a Google geocoding API key here: <https://developers.google.com/maps/documentation/geocoding/overview>
...
# geocode data
# enter a Google Maps API key to geocode
register_google(key = " ", write = TRUE)
njrecs_geocoded <- geocode(nj_recommendations$`street address`, output = "latlona", source = "google")
